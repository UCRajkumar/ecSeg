...
from .bbox_transform_tf import decode, clip_boxes, change_order
...
def draw_top_nms_proposals(pred_dict, image, min_score=0.8, draw_gt=False):
    ...
    fill_alpha = 40
    color_map = ((0, 127, 255), (0, 255, 255), (127, 255, 127), (255, 255, 0), (255, 127, 0))
    for topn, (score, proposal) in enumerate(zip(scores, proposals)):
        bbox = list(proposal)
        center = [int((bbox[3] + bbox[1])/2), int((bbox[2] + bbox[0])/2)]
        color = color_map[random.randint(0,4)]
        if edge_mask[center[0], center[1]] < 1:
            if (bbox[2] - bbox[0] <= 0) or (bbox[3] - bbox[1] <= 0):
                logger.debug('Proposal has negative area: {}'.format(bbox))
                continue

            draw.rectangle(
                bbox, fill=(color[0], color[1], color[2], 60), outline=(color[0], color[1], color[2], 150))
...
def draw_rpn_bbox_pred(pred_dict, image, top_k=10):
    """
...
def draw_rpn_bbox_pred_only(pred_dict, image, top_k=20):
    """
    For each bounding box labeled object. We want to display bbox_reg_error.
    ...
    # Perform another round of NMS for the bbox 
    bbox_tf_order = change_order(bbox_final)
    selected_indices = tf.image.non_max_suppression(
                bbox_tf_order, -loss_per_anchor,
                20, iou_threshold=0.5)
    print(selected_indices)
    # Selected_indices is a smaller tensor, we need to extract the
    # proposals and scores using it.
    nms_bbox_tf_order = tf.gather(
        bbox_tf_order, selected_indices,
        name='gather_nms_bboxs'
    )
    
    # We switch back again to the regular bbox encoding.
    bbox_final = change_order(nms_bbox_tf_order)
    
    loss_per_anchor = tf.gather(
        loss_per_anchor, selected_indices,
        name='gather_nms_loss_scores'
    )
